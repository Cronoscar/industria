...
export default class CommerceModel {
  ...
  static async getCommerceByRepresentativeDNI(dni){
    const result = await db.request()
        .input("dni", dni)
        .query(`
            SELECT 
                ...
                C.Activo,
                RC.ID_Representante,
                P.Nombre + ' ' + P.Apellido AS NombreRepresentante,
                RC.Telefono,
                P.Correo 
            FROM trade.tblComercios AS C 
            INNER JOIN users.tblRepresentantesCorporativos AS RC 
                ON C.DNI_Representante = RC.ID_Representante
            INNER JOIN users.tblPersonas AS P 
                ON RC.ID_Representante = P.ID 
            WHERE C.DNI_Representante = @dni;
        `);
    return result.recordset.length > 0 
        ? { success: true, data: result.recordset[0] } 
        : { success: false, message: "Comercio no encontrado." };
}
}
